...
현재 `V1`~`V6`는 MySQL 8.4에 적용되는 서버 스키마 변경 이력입니다.
...
## V6__create_daily_step_records.sql

- `daily_step_records`: 사용자별 날짜, 하루 총걸음, 마지막 측정 시각과 서버 생성·수정 시각을 저장합니다.
- `(user_id, record_date)` 유일 제약으로 한 사용자에게 같은 날짜의 행이 중복 생성되지 않게 합니다.
- `step_count >= 0` 검사 제약으로 애플리케이션 검증을 우회한 음수도 DB에서 차단합니다.
- 사용자 외래 키의 `ON DELETE CASCADE`로 회원 탈퇴 시 서버에 저장된 걸음 기록도 함께 삭제합니다.
